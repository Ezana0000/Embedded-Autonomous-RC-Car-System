...
void Init_Port_6(void) {
    ...
    P6SEL0 |=  R_FORWARD;     // Timer output mode
    P6SEL1 &= ~R_FORWARD;     // Timer output mode
    P6DIR  |=  R_FORWARD;     // Output configured

    // P6.2 - L_FORWARD: Left motor forward PWM signal

    P6SEL0 |=  L_FORWARD;     // PWM function selected
    P6SEL1 &= ~L_FORWARD;     // PWM function selected
    P6DIR  |=  L_FORWARD;     // Set as output

    // P6.3 - R_REVERSE: Right motor reverse PWM signal
    P6SEL0 |=  R_REVERSE;     // Timer peripheral mode
    P6SEL1 &= ~R_REVERSE;     // Timer peripheral mode
    P6DIR  |=  R_REVERSE;     // Output direction

    // P6.4 - L_REVERSE: Left motor reverse PWM signal
    P6SEL0 |=  L_REVERSE;     // PWM output enabled
    P6SEL1 &= ~L_REVERSE;     // PWM output enabled
